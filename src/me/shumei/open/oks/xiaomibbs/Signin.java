...
public class Signin extends CommonData {
	...
	public String[] start(Context ctx, boolean isAutoSign, String cfg, String user, String pwd) {
		...
		try{
			...
			//提交签到信息的URL
			String signSubmitUrl = "http://bbs.xiaomi.cn/plugin.php?id=mi_sign:sign&operation=qiandao&infloat=0&inajax=0&mobile=yes";
			...
			else
			{
				...
				res = Jsoup.connect(signSubmitUrl).data("qdmode","2").data("formhash",formhash).data("qdxq",qdxq).data("fastreply",fastreply).data("todaysay","").cookies(cookies).userAgent(UA_ANDROID).timeout(TIME_OUT).ignoreContentType(true).method(Method.POST).execute();
